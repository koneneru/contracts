...
#    curl -sSL https://github.com/protocolbuffers/protobuf/releases/download/v35.1/protoc-35.1-linux-x86_64.zip -o /tmp/protoc.zip && \
#    unzip /tmp/protoc.zip -d /usr/local && rm /tmp/protoc.zip

# Install protoc with local copy of protoc.zip
RUN apt-get update && apt-get install -y unzip git && rm -rf /var/lib/apt/lists/*

COPY protoc-35.1-linux-x86_64.zip /tmp/protoc.zip
